...
class Camera:
    def __init__(
        ...
        if save:
            ...
            self.save_path = self.save_path / f'{timestamp}_{len(os.listdir(self.save_path)) + 1:03d}'
    ...
    def run(self) -> None:
        ...
        if self.cap[0].isOpened():
            try:
                ...
                self.cap[0].release() #OpenCV의 VideoCapture 객체에서 사용 중인 자원을 해제하는 역할
                cv2.destroyAllWindows() #현재 열려 있는 모든 OpenCV 창을 닫는 역할
