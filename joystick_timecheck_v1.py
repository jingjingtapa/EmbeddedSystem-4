...
model_straight.load_state_dict(torch.load('/home/ircv6/HYU-2024-Embedded/jetracer/road_following_model_straight.pth'))
model_straight = model_straight.to(device)

model_left = models.alexnet(num_classes = 2, dropout = 0.0)
model_left.load_state_dict(torch.load('/home/ircv6/HYU-2024-Embedded/jetracer/road_following_model_left.pth'))
model_left = model_left.to(device)

model = model_straight

# model_yolo = TRTModule()
# model_yolo.load_state_dict(torch.load('240515_002.trt'))
model_yolo = YOLO('240515_002.engine', task='detect')
                ...
                for r in pred:
                    if r.boxes:
                        for box in r.boxes:
                            ...
                            #print(f'Detected: {cls_name} with confidence {score}')
                            # if cls_name == 'right':
                            #     pass
                            if cls_name == 'left':
                                print("model_left")
